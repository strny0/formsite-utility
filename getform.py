...
async def fetchContents_Results(response,page,stats,gen_files):
    print(f'API GET {response.status}: ({page}/{stats.num_pages})')
...
async def getResponseContents_Result(session,url,page,param,form_id,stats,gen_files):
    ...
    async with session.get(url,params=param) as response:
        if response.status != 200:
            print(f"Error {page}: {response.status}")
            quit()
...
async def fetchContents_Items(response,form_id,gen_items):
    print(f'API GET {response.status}: (items)')
    contents = await response.content.read()
    return contents
...
async def performAPIrequests(url,auth,param_i,param_r,form_id,stats,gen_files,gen_items,refresh_items):
    url_results = f"{url}/results"
    url_items = f"{url}/items"
    t0 = time.perf_counter()
    async with aiohttp.ClientSession(headers=auth) as session:
        first_result,stats.num_pages = await getResponseContents_Result(session,url_results,1,param_r,form_id,stats,gen_files)
        max_batch = 48
        ...
        for batch in range(0,num_batches):
            ...
            for page in range(lower_bound,upper_bound):
                response_array = asyncio.ensure_future(getResponseContents_Result(session,url_results,page,param_r,form_id,stats,gen_files))
        ...
        print(f"API: processed all calls in {t1:0.2f} seconds")
        items = await getResponseContents_Items(session,url_items,stats,param_i,form_id,gen_items)
...
def CheckVersion(check_new,event_loop):
    ...
    version_url="https://raw.githubusercontent.com/strny0/formsite-utility/main/version.md"
    future = asyncio.ensure_future(ReadFile_async(version_url))
    ...
    if check_new is True:
        ...
        if update.lower() == "y":
            asyncio.run(DownloadFile_async(f"getform-{latest_version}.py","https://raw.githubusercontent.com/strny0/formsite-utility/main/getform.py"))
...
def SetupArgumentParser(): # Sets parameters/arguments getform.py takes
    parser = argparse.ArgumentParser(
        description="This program performs an export of a specified formsite form with parameters.\n"
                    "A faster alternative to a manual export from the formsite website. Can be used for workflow automation.\n"
    ...
    parser.add_argument('--afterref', type=int, default=0, 
                        help="Get results greater than a specified Reference #. Must be an integer."
                        )
    parser.add_argument('--beforeref', type=int, default=0, 
                        help="Get results lesser than a specified Reference #. Must be an integer."
                        )
    parser.add_argument('--afterdate', type=str, default="", 
                        help="Get results after a specified date. Must be formatted as ISO 8601 UTC, YYYY-MM-DD, or YYYY-MM-DD HH:MM:SS. This date is in your local timezone."
                        )
    parser.add_argument('--beforedate', type=str, default="", 
                        help="Get results before a specified date. Must be formatted as ISO 8601 UTC, YYYY-MM-DD, or YYYY-MM-DD HH:MM:SS. This date is in your local timezone."
                        )
    parser.add_argument('--resultslabels', type=int, default=0, 
                        help="Use specific results labels for your CSV headers. Defaults to 0, which takes the first set results labels or if those are not available, default question labels."
                        )
    ...
    parser.add_argument('-o', '--output_file', nargs='?', default=False, const=Path.cwd(), 
                        help="Specify output file name and location. Defaults to export_yyyymmdd_formid.csv in the folder of the script."
                        )
    parser.add_argument('-x', '--extract_links', nargs='?',  default=False, const=Path.cwd(), #+f'\\links_{dt.now().strftime("%Y%m%d")}.txt 
                        help="If you include this flag, you will get a text file that has all links that start with formsite url stored. You can specify file name or location, for example '-x C:\\Users\\MyUsername\\Desktop\\download_links.txt'. If you don't specify a location, it will default to the folder of the script."
                        )
    parser.add_argument('-X', '--links_regex', type=str,  default='.+', 
                        help="Keep only links that match the regex you provide. Won't do anything if -x or -d arguments are not provided. Defaults to '.+'. Example usage: '-X \\.json$' would only give you files that have .json extension."
                        )
    parser.add_argument('-D', '--download_links', nargs='?',  default=False, const=Path.cwd(),
                        help="If you include this flag, all formsite links in the export will be downloaded to a folder. You can specify location, for example `-D 'C:\\Users\\My Username\\Desktop\\downloads'` . If you don't specify a location, it will default to the folder of the script."
                        )
    ...
    parser.add_argument('-g', '--generate_results_files', action="store_true",  default=False, 
                        help="If you use this flag, program will output raw results in json format from API requests. Useful for debugging purposes."
                        )
...
async def FileDownloadLoop(links, files_url, destination):
    ...
    try:
        ...
        print()
    for url in links:
        filename = destination.as_posix()+"/"+url.replace(files_url,'')
        tasks.append(DownloadFile_async(filename,url))
    await asyncio.wait(tasks)
    t = time.perf_counter() - t0
    print(f"Files downloaded in in {t:0.2f} seconds.\n")
...
def WriteResults(args, dataframe, encoding, separator, line_terminator, quoting, date_format):
    if args.output_file != False:
        ...
        if arguments.output_file == Path.cwd().resolve():
            output_file = "export_%s_%s.csv" % (dt.strftime(dt.now(),date_format),args.form) # "%Y-%m-%d %H-%M-%S"
        else:
        ...
        print('Rows:      %d ' % dataframe.shape[0])
...
if __name__ == '__main__':
    ...
    # Generates initial variables from arguments provided
    ...
    future = asyncio.ensure_future(performAPIrequests(url_forms,authHeader,itemsParams,resultsParams,arguments.form,statistics,arguments.generate_results_files,arguments.no_items,arguments.refresh_headers))
    ...
    if arguments.extract_links != False or arguments.download_links != False:
        ...
        if arguments.extract_links != False:
            if arguments.extract_links == os.getcwd():
                x_filename = f"links_{dt.strftime(dt.now(),'%Y%m%d')}_{arguments.form}.txt"
            else:
        ...
        if arguments.download_links != False:
            future = asyncio.ensure_future(FileDownloadLoop(links,url_files,arguments.download_links))
            loop.run_until_complete(future)
    # End
    print('\nAPI calls: %d' % statistics.num_pages)
    quit()
=======
#!/usr/bin/python3
# Script: getform.py
